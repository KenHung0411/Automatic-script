...
def main():
	...
	cursor.execute("alter session set nls_date_format='YYYY/MM/DD HH24:mi:ss'")
        sel_qry = '''select f.ALARM_NUMBER,f.alarm_time,f.CANCEL_TIME,f.dn,f.text 
	            from fx_alarm f 
	            where 1=1
	            and alarm_number not in (599)
	            --and alarm_status=1
	            and dn like '%OMC%' '''
    ...
    email_sub = 'LGC & SRRi internal alarms check'
    html_text = make_blank_html()
    email_sub = ' LGC & SRRi internal alarms check '+def_check_date
    print email_sub
